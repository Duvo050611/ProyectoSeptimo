...
$pdf->Cell(110, 9, utf8_decode('NOTA EXPLORACION SEGMENTO POSTERIOR'), 1, 0, 'C');
$pdf->SetX(160);
$pdf->SetFont('Arial', '', 6);
...
$sql_segmento = "SELECT * FROM segmento_post WHERE id_exp = $id_exp AND id_atencion = $id_atencion ORDER BY id DESC LIMIT 1";
$result_segmento = $conexion->query($sql_segmento);

while ($row_seg = $result_segmento->fetch_assoc()) {
    $bajo_dilatacion = $row_seg['bajo_dilatacion'];
    $bajo_do = $row_seg['bajo_do'];
    $bajo_dx = $row_seg['bajo_dx'];
    $bajo_total = $row_seg['bajo_total'];
    $bajo_media = $row_seg['bajo_media'];
    $bajo_no_dilata = $row_seg['bajo_no_dilata'];
    $no_valorable_od = $row_seg['no_valorable_od'];
    $no_valorable_oi = $row_seg['no_valorable_oi'];
    $vitreo_od = $row_seg['vitreo_od'];
    $vitreo_oi = $row_seg['vitreo_oi'];
    $nervio_optico_od = $row_seg['nervio_optico_od'];
    $nervio_optico_oi = $row_seg['nervio_optico_oi'];
    $retina_periferica_od = $row_seg['retina_periferica_od'];
    $retina_periferica_oi = $row_seg['retina_periferica_oi'];
    $macula_od = $row_seg['macula_od'];
    $macula_oi = $row_seg['macula_oi'];
    $observaciones_dibujo = $row_seg['observaciones_dibujo'];
    $creado_en = $row_seg ['creado_en'];
}
...
$pdf->Ln(10);
$pdf->SetFont('Arial', 'B', 9);
$pdf->Cell(190, 6, utf8_decode('Segmento Posterior'), 1, 1, 'C');

$pdf->SetFont('Arial', '', 8);
$pdf->Cell(95, 6, utf8_decode('¿Dilató el paciente?'), 1);
$pdf->Cell(95, 6, ucfirst($bajo_dilatacion), 1, 1);


 
$pdf->Cell(95, 6, utf8_decode('Valorable OD'), 1);
$pdf->Cell(95, 6, ($no_valorable_od == 1 ? 'El ojo no ha podido ser valorado' : 'Ha sido valorado con exito'), 1);
$pdf->Ln();

$pdf->Cell(95, 6, utf8_decode('Valorable OI'), 1);
$pdf->Cell(95, 6, ($no_valorable_oi == 1 ? 'El ojo no ha podido ser valorado' : 'Ha sido valorado con exito'), 1);
$pdf->Ln();


$pdf->Cell(47.5, 6, utf8_decode('Vítreo OD'), 1);
$pdf->Cell(47.5, 6, $vitreo_od, 1);
$pdf->Cell(47.5, 6, utf8_decode('Vítreo OI'), 1);
$pdf->Cell(47.5, 6, $vitreo_oi, 1);
$pdf->Ln();

$pdf->Cell(47.5, 6, utf8_decode('Nervio óptico OD'), 1);
$pdf->Cell(47.5, 6, $nervio_optico_od, 1);
$pdf->Cell(47.5, 6, utf8_decode('Nervio óptico OI'), 1);
$pdf->Cell(47.5, 6, $nervio_optico_oi, 1);
$pdf->Ln();

$pdf->Cell(47.5, 6, utf8_decode('Retina periférica OD'), 1);
$pdf->Cell(47.5, 6, $retina_periferica_od, 1);
$pdf->Cell(47.5, 6, utf8_decode('Retina periférica OI'), 1);
$pdf->Cell(47.5, 6, $retina_periferica_oi, 1);
$pdf->Ln();

$pdf->Cell(47.5, 6, utf8_decode('Mácula OD'), 1);
$pdf->Cell(47.5, 6, $macula_od, 1);
$pdf->Cell(47.5, 6, utf8_decode('Mácula OI'), 1);
$pdf->Cell(47.5, 6, $macula_oi, 1);
$pdf->Ln();

$pdf->Ln(5);
$pdf->SetFont('Arial', 'I', 7);
$pdf->Cell(60, 5, utf8_decode('Fecha de Registro: ' . date('d/m/Y H:i', strtotime($creado_en))), 0, 1);
